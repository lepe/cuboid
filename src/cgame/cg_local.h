...
#define MAX_CUBOID_TEXTURES 8
#define MAX_CUBOID_FRAGMENTS 4
#define MAX_CUBOID_SOUNDS 8

typedef struct
{
  int          textureCount;
  qhandle_t    textures[ MAX_CUBOID_TEXTURES ];
  float        textureThresholds[ MAX_CUBOID_TEXTURES ];
  qboolean     useCracks;
 
  int          fragmentCount;
  qhandle_t    fragments[ MAX_CUBOID_FRAGMENTS ];
  
  int          painSoundCount;
  sfxHandle_t  painSounds[ MAX_CUBOID_SOUNDS ];
  
  int          destroySoundCount;
  sfxHandle_t  destroySounds[ MAX_CUBOID_SOUNDS ];
  float        destroySoundThresholds[ MAX_CUBOID_SOUNDS ];
  
  sfxHandle_t  idleSound;
} cuboidInfo_t;
...
typedef struct
{
  ...
  qhandle_t   cuboidGreenBuildShader;
  qhandle_t   cuboidAxis;
  qhandle_t   cuboidAlienPrebuild;
  
  sfxHandle_t cuboidErrorSound;
  sfxHandle_t cuboidResizeSoundA;
  sfxHandle_t cuboidResizeSoundB;
  sfxHandle_t cuboidRotateSound; 
  sfxHandle_t cuboidAxisChangeSound;
  
  qhandle_t   splashLogo[15];
  qhandle_t   splashLeft;
  qhandle_t   splashRight;
...
extern  buildableInfo_t cg_buildables[ CUBOID_FIRST ];
extern  cuboidInfo_t    cg_cuboids[ CUBOID_TYPES ];
...
// cg_buildable.c
//
void        CG_GhostBuildable( buildable_t buildable, vec3_t cuboidSize );
...
// mod version data
#define SPLASH_VERSION          "0.1.9.4 (July 14th, 2014)"
